...
#ifdef QEMU_PLUGIN_HAS_REG_CB
static void vcpu_reg_access(unsigned int vcpu_index, qemu_plugin_reginfo_t info,
                            uint64_t vaddr, void* userdata) {
    bool is_st = qemu_plugin_mem_is_store(info);
    if (is_st) {
        ++ st_count;
    ...
    printf("cpu:%d, pc:%p, reg_val:%lx, size:%d, is_st:%d\n", vcpu_index,
            userdata, vaddr, 1 << qemu_plugin_mem_size_shift(info), is_st);
}
#endif
...
#ifdef QEMU_PLUGIN_HAS_REG_CB
        qemu_plugin_register_vcpu_reg_cb(insn, vcpu_reg_access, QEMU_PLUGIN_CB_NO_REGS, QEMU_PLUGIN_REG_RW, (void*)addr);
#endif
    }
}
